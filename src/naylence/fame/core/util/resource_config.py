...
class ResourceConfig(BaseModel):
    ...
    def _recursive_model_dump(self, *args, by_alias: bool | None = None, **kwargs):
        ...
        for name, field in self.__class__.model_fields.items():
            ...
            field_name = field.alias if by_alias is True and field and field.alias else name
            if isinstance(value, ResourceConfig):
                data[field_name] = value._recursive_model_dump(*args, by_alias=by_alias, **kwargs)
                ...
                data[field_name] = {
                    k: v._recursive_model_dump(*args, by_alias=by_alias, **kwargs)
                    if isinstance(v, ResourceConfig)
                    else v.model_dump(*args, by_alias=by_alias, **kwargs)
                    if isinstance(v, BaseModel)
                    else v
        ...
        def _dispatch(value, next_, info):
            ...
            if (isinstance(value, dict) and "type" in value):
                ...
                if sub and sub is not cls and issubclass(sub, cls):
                    ...
                    return sub.model_validate(value, context=info.context if info else None)
